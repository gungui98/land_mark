...
std::vector<cv::Point3d> get_3d_model_points()
{
    ...
    //static 3d coordinate
    modelPoints.push_back(cv::Point3d(0.0f, 0.0f, 0.0f)); // Nose
    modelPoints.push_back(cv::Point3d(0.0f, -330.0f, -65.0f)); // chin
    modelPoints.push_back(cv::Point3d(-225.0f, 170.0f, -135.0f)); // left eye
    modelPoints.push_back(cv::Point3d(225.0f, 170.0f, -135.0f)); // right eye
    modelPoints.push_back(cv::Point3d(-150.0f, -150.0f, -125.0f)); // left mouth
    modelPoints.push_back(cv::Point3d(150.0f, -150.0f, -125.0f)); // right mount
...
//left eye
{
                std::vector<cv::Point3d> glass_point3D;
                std::vector<cv::Point2d> glass_point2D;
                //upper left
                glass_point3D.push_back(cv::Point3d(-235.0f, 240.0f, -135.0f));
                //lower left
                glass_point3D.push_back(cv::Point3d(-235.0f, 120.0f, -135.0f));
                //upper right
                glass_point3D.push_back(cv::Point3d(-80.0f, 120.0f, -135.0f));
                //lower right
                glass_point3D.push_back(cv::Point3d(-80.0f, 240.0f, -135.0f));

				cv::projectPoints(glass_point3D, rotation_vector, translation_vector, camera_matrix, dist_coeffs, glass_point2D);
				std::vector <cv::Point> points;
				for (cv::Point2d p:glass_point2D)
                {
                    points.push_back(cv::Point(p.x, p.y));
                };
								cv::polylines(im, points, true, cv::Scalar(0,0,255), 2, 16);
...
//right eye
{
        ...
        //lower left
        glass_point3D.push_back(cv::Point3d(80.0f, 240.0f, -135.0f));
        // convert 3d point to corresponding 2d point
        cv::projectPoints(glass_point3D, rotation_vector, translation_vector, camera_matrix, dist_coeffs, glass_point2D);
        //draw bounding box
        std::vector <cv::Point> points;
        for (cv::Point2d p:glass_point2D)
...
// glasses bridge
{
    ...
    glass_point3D.push_back(cv::Point3d(-80.0f, 180.0f, -135.0f));
    glass_point3D.push_back(cv::Point3d(80.0f, 180.0f, -135.0f));
    ...
    cv::polylines(im, points, false, cv::Scalar(0,0,255), 2, 16);
}
